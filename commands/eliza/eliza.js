...
ElizaBot.prototype.reset = function() {
	...
	this.mem=[];
	this.lastchoice=[];
	for (var k=0; k<data.elizaKeywords.length; k++) {
		this.lastchoice[k]=[];
...
ElizaBot.prototype._init = function() {
	...
	if ((data.elizaSynons) && (typeof data.elizaSynons == 'object')) {
		for (var i in data.elizaSynons) synPatterns[i]='('+i+'|'+data.elizaSynons[i].join('|')+')';
	}
	// check for keywords or install empty structure to prevent any errors
	if ((!data.elizaKeywords) || (typeof data.elizaKeywords.length == 'undefined')) {
		data.elizaKeywords=[['###',0,[['###',[]]]]];
	}
	...
	for (var k=0; k<data.elizaKeywords.length; k++) {
		var rules=data.elizaKeywords[k][2];
		data.elizaKeywords[k][3]=k; // save original index for sorting
	...
	data.elizaKeywords.sort(this._sortKeywords);
	// and compose regexps and refs for pres and posts
	ElizaBot.prototype.pres={};
	ElizaBot.prototype.posts={};
	if ((data.elizaPres) && (data.elizaPres.length)) {
		var a=new Array();
		for (var i=0; i<data.elizaPres.length; i+=2) {
			a.push(data.elizaPres[i]);
			ElizaBot.prototype.pres[data.elizaPres[i]]=data.elizaPres[i+1];
		}
	...
	if ((data.elizaPosts) && (data.elizaPosts.length)) {
		var a=new Array();
		for (var i=0; i<data.elizaPosts.length; i+=2) {
			a.push(data.elizaPosts[i]);
			ElizaBot.prototype.posts[data.elizaPosts[i]]=data.elizaPosts[i+1];
		}
	...
	if ((!data.elizaQuits) || (typeof data.elizaQuits.length == 'undefined')) {
		data.elizaQuits=[];
	}
	// done
...
ElizaBot.prototype.transform = function(text) {
	...
	for (var i=0; i<parts.length; i++) {
		...
		if (part!='') {
			...
			for (var q=0; q<data.elizaQuits.length; q++) {
				if (data.elizaQuits[q]==part) {
					this.quit=true;
			...
			for (var k=0; k<data.elizaKeywords.length; k++) {
				if (part.search(new RegExp('\\b'+data.elizaKeywords[k][0]+'\\b', 'i'))>=0) {
...
ElizaBot.prototype._execRule = function(k) {
	...
	for (var i=0; i<decomps.length; i++) {
		...
		if (m!=null) {
			...
			var rpl=reasmbs[ri];
			if (this.debug) alert('match:\nkey: '+data.elizaKeywords[k][0]+
				'\nrank: '+data.elizaKeywords[k][1]+
				'\ndecomp: '+decomps[i][0]+
				'\nreasmb: '+rpl+
				'\nmemflag: '+memflag);
...
ElizaBot.prototype._getRuleIndexByKey = function(key) {
	for (var k=0; k<data.elizaKeywords.length; k++) {
		if (data.elizaKeywords[k][0]==key) return k;
	}
	return -1;
}
...
ElizaBot.prototype.getFinal = function() {
	return data.elizaFinals[Math.floor(Math.random()*data.elizaFinals.length)];
}

ElizaBot.prototype.getInitial = function() {
	return data.elizaInitials[Math.floor(Math.random()*data.elizaInitials.length)];
}
